/**
 * AdbloquRs: YouTube IMA SDK shim
 *
 * YouTube's web player drives video ads through Google's IMA SDK
 * (imasdk.googleapis.com/js/sdkloader/ima3.js). If that SDK is neutralized
 * the player never renders an ad: the ad break is requested, instantly
 * "completes", and playback returns to content — no muted ad, no skip-button
 * dance, no seek glitches. This is the same approach AdGuard and uBlock
 * Origin use.
 *
 * The shim below is the battle-tested implementation from uBlock Origin
 * (itself based on Mozilla's webcompat shim). It is shipped verbatim (with
 * the original license header) so YouTube's IMA integration sees the full
 * API surface it expects.
 *
 * How this extension wires it:
 *  - manifest.json registers this file as a content script on
 *    *://*.youtube.com/* and *://*.youtube-nocookie.com/* with
 *    `"world": "MAIN"` and `"run_at": "document_start"`, so it runs in the
 *    page's own JS world before YouTube's player scripts load.
 *  - rules.json statically blocks imasdk.googleapis.com script/XHR requests
 *    initiated by YouTube pages, so the real SDK can never load and
 *    overwrite `window.google.ima`.
 *  - youtube-ad-remover.js (isolated world) remains as a DOM-level fallback
 *    for anything that slips through.
 *
 * Source: https://github.com/gorhill/uBlock/blob/master/src/web_accessible_resources/google-ima.js
 */
